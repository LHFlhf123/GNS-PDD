...
sys.path.append(os.path.join(current_dir, 'compare'))
from compare import compare_vsa
...
class CBinDiff:
    ...
    def diff(self):
        try:
            t0 = time.time()
            # 先把两个目录中 同名 的 stripped 函数直接当作已匹配（mapping[func] = func），这些函数不会再参与后续算法。
            self.find_initial_matches()
            log_refresh("Find matches along the callgraph...")
            #  从初始匹配的函数出发，沿调用图（callee 与 caller 方向）做 1-hop 邻居匹配：
            self.match_along_callgraph_nhop(1)
            # 分 4 个阶段，尝试用不同子集做剩余匹配
...
def diff_two_files(db1, db2, out, with_gt):
    #  显示要对比的两个特征目录 db1、db2 以及输出目录 out
    print(db1, db2, out, with_gt)
    # 构造 Diff 对象：CBinDiff(db1)，db_name 字段暂时存储第一个目录。
    bd = CBinDiff(db1)
    # 加载调用图和特征文件,oad_callgraph 解析两个目录下的
    # callgraph.txt
    # addr2funcname_stripped.txt + addr2funcname.txt（ground truth 如果 with_gt=True）
    # 还会尝试加载每个目录下的 embedding 文件（.dat 或 .npy），以便后面基于语义向量做初步匹配。
    bd.load_callgraph(db1, db2, with_gt)
    #  执行 diff：调用 bd.diff()，进入主匹配流程
    bd.diff()
    if not (os.path.exists(out)):
        os.mkdir(out)
    # 调用 bd.evaluate(...) 把最终配对结果输出到 matched_functions.txt 并打印 Precision/Recall
