...
class DGMC(torch.nn.Module):
    ...
    def forward(self, x_s, edge_index_s, edge_attr_s, batch_s, x_t,
                edge_index_t, edge_attr_t, batch_t, y=None, jump_index_s=None, jump_index_t=None):
        r"""
        Args:
        ...
        """
        # print(f"[DGMC·FUSE] #jumps_s={len(jump_pairs_s)}, #jumps_t={len(jump_pairs_t)}")
        ...
        # if jump_pairs_s:
        #     x_s = x_s.clone()  # 防止原地修改后影响反向计算
        #     for src_idx, tgt_idx in jump_pairs_s:
        #         fused = self.jump_fusion( torch.cat([x_s[tgt_idx], x_s[src_idx]], dim=-1))
        #         x_s[tgt_idx] = fused
        #
        # if jump_pairs_t:
        #     x_t = x_t.clone()
        #     for src_idx, tgt_idx in jump_pairs_t:
        #         fused = self.jump_fusion(torch.cat([x_t[tgt_idx], x_t[src_idx]], dim=-1))
        #         x_t[tgt_idx] = fused
        # # —— 2) 正常的两次 GNN 传播流程 ——
        ...
        if self.k < 1:
            ...
            # --------- DGMC Diagnostic: initial correspondence stats ----------
            # 打印B在图匹配第一阶段结束后的节点数
            ...
            try:
                ...
                if 'S_0' in locals():
                    ...
                    _S_init = locals()['corr']
                elif 'S_curr' in locals():
                    _S_init = locals()['S_curr']
                # 如果上述都没命中，可以手动替换变量名为你的初始相似矩阵名称

                if _S_init is not None:
                    with _torch.no_grad():
                        _S_cpu = _S_init.detach().cpu()
                        # 两种统计：argmax-based (哪几个源有显著最大匹配) 与 非零条目(>eps)
                        eps = 1e-6
                        # 以argmax计：每个 source 的最大值是否 > threshold
                        maxvals_src = _S_cpu.max(dim=1).values
                        matched_src_argmax = int((maxvals_src > 1e-3).sum().item())  # 1e-3 可调整为 0.001 或 0.5
                        maxvals_tgt = _S_cpu.max(dim=0).values
                        matched_tgt_argmax = int((maxvals_tgt > 1e-3).sum().item())

                        matched_src_nonzero = int((_S_cpu.abs() > eps).any(dim=1).sum().item())
                        matched_tgt_nonzero = int((_S_cpu.abs() > eps).any(dim=0).sum().item())
                        total_pairs = int((_S_cpu.abs() > eps).sum().item())

                        print(f"[DGMC·INIT] S_shape={tuple(_S_cpu.shape)} "
                              f"matched_src_argmax={matched_src_argmax} matched_tgt_argmax={matched_tgt_argmax} "
                              f"matched_src_nonzero={matched_src_nonzero} matched_tgt_nonzero={matched_tgt_nonzero} "
                              f"total_nonzero_pairs={total_pairs}")
                else:
                    print(
                        "[DGMC·INIT] Warning: cannot find initial similarity matrix (S/S_L/corr). Adjust variable name.")
            except Exception as _e:
                print("[DGMC·INIT][ERR]", _e)
            # ------------------------------------------------------------------
            ...
            # --------- DGMC Diagnostic: after neighbor-consensus (final stats) ----------
            #  打印B图匹配第二阶段结束后的节点数
            import torch as _torch
            try:
                _S_final = None
                if 'S_L' in locals():
                    ...
                    _S_final = locals()['corr']
                elif 'S_curr' in locals():
                    _S_final = locals()['S_curr']

                if _S_final is not None:
                    with _torch.no_grad():
                        _S_cpu = _S_final.detach().cpu()
                        eps = 1e-6
                        matched_src = int((_S_cpu.abs() > eps).any(dim=1).sum().item())
                        matched_tgt = int((_S_cpu.abs() > eps).any(dim=0).sum().item())
                        total_nonzero = int((_S_cpu.abs() > eps).sum().item())
                        print(
                            f"[DGMC·FINAL] S_shape={tuple(_S_cpu.shape)} matched_src={matched_src} matched_tgt={matched_tgt} total_nonzero={total_nonzero}")
                else:
                    print("[DGMC·FINAL] Warning: cannot find final similarity matrix variable (S/S_L/corr).")
            except Exception as _e:
                print("[DGMC·FINAL][ERR]", _e)
            # -----------------------------------------------------------------------
