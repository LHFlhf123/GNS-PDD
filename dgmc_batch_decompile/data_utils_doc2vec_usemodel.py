...
# from TestOwnDataPreTraining import train
...
class Corpus(object):
    ...
    def get_data(self, node_label_file_1,edge_file_1,node_label_file_2,edge_file_2,training_file,func_matching_file,subject_dir,pretrained_subject, with_gt):
        ...
        # ——— 修复：用零向量初始化，保证每个 entry 都是同样 shape 的 ndarray ———
        vec_dim = model.vector_size
        embedding_seq_1 = [np.zeros(vec_dim, dtype=float)
                           for _ in range(last_node_seq_1 + 1)]
        embedding_seq_2 = [np.zeros(vec_dim, dtype=float)
                           for _ in range(last_node_seq_2 + 1)]
        ...
        embedding_1=embedding_seq_1
        embedding_2=embedding_seq_2
        ...
        # if last_node_seq_1 < 50000 and last_node_seq_2 < 50000:
        #     # embedding_1 是 list of np.ndarray already
        #     # edges_1, edges_2, train_y 都是 Python list of np.ndarray
        #     return (
        #         [np.stack(embedding_1, axis=0)],
        #         [np.array(edges_1, dtype=np.int64)],
        #         [np.stack(embedding_2, axis=0)],
        #         [np.array(edges_2, dtype=np.int64)],
        ...
        while has_next:
            ...
            # determine embedding dim for embedding_seq_1
            def _get_emb_dim(embedding_seq):
                # embedding_seq could be: numpy.ndarray (N,D) or list/dict of vectors
                if isinstance(embedding_seq, np.ndarray):
                    if embedding_seq.ndim == 2:
                        return embedding_seq.shape[1]
                # dict-like
                try:
                    # dict: next(iter(.values()))
                    if hasattr(embedding_seq, "values"):
                        sample = next(iter(embedding_seq.values()))
                    else:
                        # list-like
                        sample = embedding_seq[0] if len(embedding_seq) > 0 else None
                except Exception:
                    sample = None
                if sample is None:
                    # fallback: set to args.dim if available; otherwise raise
                    # try to use a global/default dim if exists
                    raise ValueError("embedding sequence appears empty or malformed; can't infer embedding dim.")
                return int(np.array(sample).shape[0])

            # before building arrays, get emb dims
            emb_dim_1 = _get_emb_dim(embedding_seq_1)
            emb_dim_2 = _get_emb_dim(embedding_seq_2)

            # build arr for graph1
            if len(node_ids1) > 0:
                arr1 = np.stack([embedding_seq_1[nid] for nid in node_ids1], axis=0)
            else:
                # empty graph: create (0, D) array (float32 to match typical embedding dtype)
                arr1 = np.zeros((0, emb_dim_1), dtype=np.float32)

            # build arr for graph2
            if len(node_ids2) > 0:
                arr2 = np.stack([embedding_seq_2[nid] for nid in node_ids2], axis=0)
            else:
                arr2 = np.zeros((0, emb_dim_2), dtype=np.float32)
            embedding_1_list.append( arr1)
            embedding_2_list.append( arr2)
            ...
            edges_1_list.append(np.array(edges1, dtype=np.int64))
            edges_2_list.append(np.array(edges2, dtype=np.int64))
            ...
            train_y_list.append(np.array(trainy, dtype=np.int64))
